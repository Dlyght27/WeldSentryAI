...
# --- FEATURE EXTRACTION (From second app - CORRECT VERSION) ---
def extract_features(img):
    """
    Extract HOG, LBP, and GLCM features from welding image.
    This matches the training pipeline from the second app.
    """
    if len(img.shape) == 3:
    ...
    combined_features = np.concatenate([hog_features, hist_lbp, glcm_features])
    return combined_features.reshape(1, -1)

# --- Image Enhancement (From second app) ---
def preprocess_for_analysis(image_np):
    """
    Apply consistent preprocessing to improve detection accuracy.
    """
...
def get_prediction_corrected(image, use_enhancement):
    """Generate prediction using the correct feature extraction from second app."""
    if model is None:
    ...
    if use_enhancement:
        img_np = preprocess_for_analysis(img_np)
    else:
        if len(img_np.shape) == 3:
            img_np = cv2.cvtColor(img_np, cv2.COLOR_RGB2GRAY)
    
    # Extract features using the CORRECT method from second app
    ...
    try:
        ...
        try:
            ...
            confidence = 0.8 if pred == 0 else 0.7
        
        # Map prediction to labels (0=Good, 1=Defective based on second app)
        label = "Good Weld" if pred == 0 else "Defective Weld"
        
        return label, confidence, pred, features.shape[1]
        
    except Exception as e:
        st.error(f"Prediction error: {e}")
        return "Prediction error", 0.0, None, None
            ...
            if not is_valid:
                ...
                if use_enhancement:
                    ...
                    with col2:
                        ...
                        enhanced_np = preprocess_for_analysis(img_np)
                ...
                with st.spinner("🔄 Analyzing weld quality... Please wait..."):
                    ...
                    label, confidence, pred, feature_count = get_prediction_corrected(original_image, use_enhancement)
                
                # Debug info in expander
                with st.expander("🔍 Technical Details"):
                    st.write(f"**Prediction:** {pred} (0=Good, 1=Defective)")
                    st.write(f"**Feature Count:** {feature_count}")
                    st.write(f"**Model Used:** welding_defect_baseline_model.pkl")
                    st.write(f"**Image Enhancement:** {'Enabled' if use_enhancement else 'Disabled'}")
